...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_Start
********************************************************************************
*
* Summary:
...
void APPS_ADC_IntClock_Start(void) 
{
    ...
    APPS_ADC_IntClock_CLKEN |= APPS_ADC_IntClock_CLKEN_MASK;
	APPS_ADC_IntClock_CLKSTBY |= APPS_ADC_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: APPS_ADC_IntClock_Stop
********************************************************************************
*
* Summary:
...
void APPS_ADC_IntClock_Stop(void) 
{
    ...
    APPS_ADC_IntClock_CLKEN &= (uint8)(~APPS_ADC_IntClock_CLKEN_MASK);
	APPS_ADC_IntClock_CLKSTBY &= (uint8)(~APPS_ADC_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((APPS_ADC_IntClock_CLKEN & APPS_ADC_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = APPS_ADC_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = APPS_ADC_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* APPS_ADC_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(APPS_ADC_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(APPS_ADC_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = APPS_ADC_IntClock_GetSourceRegister();
    uint16 oldDivider = APPS_ADC_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = APPS_ADC_IntClock_CLKEN & APPS_ADC_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(APPS_ADC_IntClock_DIV_PTR, clkDivider);
                APPS_ADC_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                APPS_ADC_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(APPS_ADC_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((APPS_ADC_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(APPS_ADC_IntClock_DIV_PTR, clkDivider);
				APPS_ADC_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(APPS_ADC_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: APPS_ADC_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    APPS_ADC_IntClock_MOD_SRC |= modeBitMask & (uint8)APPS_ADC_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: APPS_ADC_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    APPS_ADC_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(APPS_ADC_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: APPS_ADC_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return APPS_ADC_IntClock_MOD_SRC & (uint8)(APPS_ADC_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: APPS_ADC_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = APPS_ADC_IntClock_GetDividerRegister();
    uint8 oldSrc = APPS_ADC_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        APPS_ADC_IntClock_MOD_SRC |= CYCLK_SSS;
        APPS_ADC_IntClock_MOD_SRC =
            (APPS_ADC_IntClock_MOD_SRC & (uint8)(~APPS_ADC_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return APPS_ADC_IntClock_MOD_SRC & APPS_ADC_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: APPS_ADC_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    APPS_ADC_IntClock_PHASE = clkPhase & APPS_ADC_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: APPS_ADC_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return APPS_ADC_IntClock_PHASE & APPS_ADC_IntClock_PHASE_MASK;
}
