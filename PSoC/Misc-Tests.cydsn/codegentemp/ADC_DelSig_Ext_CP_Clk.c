...
void ADC_DelSig_Ext_CP_Clk_Stop(void) 
{
    ...
    ADC_DelSig_Ext_CP_Clk_CLKEN &= (uint8)(~ADC_DelSig_Ext_CP_Clk_CLKEN_MASK);
	ADC_DelSig_Ext_CP_Clk_CLKSTBY &= (uint8)(~ADC_DelSig_Ext_CP_Clk_CLKSTBY_MASK);
}
...
{
    if ((ADC_DelSig_Ext_CP_Clk_CLKEN & ADC_DelSig_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
...
#endif /* ADC_DelSig_Ext_CP_Clk__CFG3 */
    ...
    if (clkDivider != oldDivider)
    {
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                ADC_DelSig_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_DelSig_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    ADC_DelSig_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)ADC_DelSig_Ext_CP_Clk_MODE_MASK;
}
...
{
    ADC_DelSig_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_DelSig_Ext_CP_Clk_MODE_MASK));
}
...
{
    return ADC_DelSig_Ext_CP_Clk_MOD_SRC & (uint8)(ADC_DelSig_Ext_CP_Clk_MODE_MASK);
}
...
void ADC_DelSig_Ext_CP_Clk_SetSourceRegister(uint8 clkSource) 
{
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_DelSig_Ext_CP_Clk_MOD_SRC =
            (ADC_DelSig_Ext_CP_Clk_MOD_SRC & (uint8)(~ADC_DelSig_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
